...
import numpy as np
from board_util import where1d, BORDER, BLACK, EMPTY, WHITE
from board_util import GO_COLOR, GO_POINT, PASS, NULLPOINT
    ...
    for pt in range(board.maxpoint):
        ...
        if color == BLACK:
            ...
            for p_int in empty_points:
                ...
                if board.find_neighbor_of_color(p, BLACK) != NULLPOINT:
                    black_flag = True
                if board.find_neighbor_of_color(p, WHITE) != NULLPOINT:
                    white_flag = True
